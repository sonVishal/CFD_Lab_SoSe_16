	...
	*/
	int p_computeNeighborCellOffset(int outer, int inner, int fixedValue,
		...
		*/
		...
		int extractEdge(double sendBuffer[], double const * const collideField, t_iterParaEdge const * const iterPara, t_procData const * const procData,
			const int edge, const int index){
				...
				for(int varIdx = 1; varIdx <= endIndex; ++varIdx){
					...
					sendBuffer[currentIndexBuff++] = collideField[currentIndexField+index];
				}
		}
