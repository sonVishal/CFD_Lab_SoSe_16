	...
	* adapted to xlength[3] */
	...
	for (i = 0; i < totalSize; i++) {
		...
		if (flagField[i].type == FLUID) {
			...
			if (flagField[i+xylen2+xlen2].type != -1)
			...
			if (flagField[i+xylen2+1].type != -1)
