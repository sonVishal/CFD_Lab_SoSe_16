...
/* Function to get the discrete Laplacian of any matrix U
at point i,j */
...
void calculate_dt(
	...
	*dt = tau*min;
	if(*dt <= 0){
		printf("WARNING: dt has invalid value: %f \n", *dt);
	}
}

/* Calculate the updated U and V according to equations 8 and 9*/
void calculate_uv(
    double dt,
    double dx,
    double dy,
