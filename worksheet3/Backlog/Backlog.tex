...
\section{Backlog}
\begin{enumerate}
\item Tool to create scenarios (domain + obstacles

\begin{enumerate}
\item \td{Group disussion:}
what "tool" are we using to create domains? (consider: should be easy to "integrate" and flexible enough to build "complex" scenarios)
\item[] \textbf{Programmatically}... Matlab/Python - just create a boolean matrix, and tell which "pixels" are obstacles, matrix can also be plotted easily, etc.
\item[] \textbf{Graphically}... "Paint"-like drawing and then reading via a script?
\item \td{Group decision}
Do we use "read\_pgm"? (see page 12 (3) \url{https://en.wikipedia.org/wiki/Netpbm_format})
\end{enumerate}

\item Adapt parameter file, read parameter and check for correctness
...
\item adapt initializeFields according to the current problem we are simulating; see page 12 (3)
...
\item using of \qm{read\_pmg} can be used to initialize flagField \todo{Decide !!}
\item Check for correctness, is there is any illegal bondary (too thin)?
\end{enumerate}
